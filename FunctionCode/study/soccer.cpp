...
void search(Team team[], int n, const vector<Game> &game, unsigned int gi)
{
  ...
  int pts1 = team[index1].points, pts2 = team[index2].points; // 백업

  // team 1 wins
  team[index1].update(1, 0); // 1팀 승
  team[index2].update(0, 1); // 2팀 패
...
bool do_case(bool first)
{
  ...
  // 연산부
  search(team, n, game, 0); // (팀 리스트, 팀 수, 게임리스트, 인덱스?)
